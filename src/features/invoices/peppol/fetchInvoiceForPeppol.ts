...
import type { PeppolInvoice, PeppolInvoiceItem } from "./buildPeppolInvoiceXml";

export type FetchedPeppolInvoice = {
  invoice: PeppolInvoice;
  items: PeppolInvoiceItem[];
};

/**
 * Fetches all data required to build a Peppol UBL invoice XML.
 * Single source of truth — used by both exportPeppolInvoiceXml and generatePeppolInvoiceFile.
 */
export async function fetchInvoiceForPeppol(
  invoiceId: string
): Promise<FetchedPeppolInvoice> {
  ...
  return {
    invoice: invoiceRes.data as PeppolInvoice,
    items: (itemsRes.data ?? []) as PeppolInvoiceItem[],
  };
}